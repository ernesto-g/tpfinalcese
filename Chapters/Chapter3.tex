...
El módulo pyb tiene en su interior las clases que representan los periféricos del microcontrolador. Una vez incluído el módulo, se pueden utilizar sus clases para crear objetos.
...
También se han agregado porciones de código para validación de rangos o para agregar argumentos, como se muestra en el algoritmo \ref{lst:uarthal} en donde se define la función para enviar datos por el puerto serie.
...
\begin{itemize}
	...
	\item \textbf{XMODEM}: biblioteca que implementa el protocolo \textit{xmodem}\footnote{xmodem: Protocolo simple de transferencia de datos por medio de paquetes de 128bytes.} y permite el envío del archivo a la placa.	
\end{itemize}
